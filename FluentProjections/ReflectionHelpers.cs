...
namespace FluentProjections
{
    public static class ReflectionHelpers
    {
        ...
        public static PropertyInfo GetMessagePropertyInfo<TMessage, TProjection, TValue>(
            Expression<Func<TProjection, TValue>> projectionProperty)
        {
            PropertyInfo propertyInfo = typeof (TMessage).GetProperty(GetPropertyInfo(projectionProperty).Name);
            if (propertyInfo == null)
            {
                throw new ArgumentOutOfRangeException("projectionProperty", "No associated message property found.");
            }
            return propertyInfo;
        }
        ...
        public static TValue GetPropertyValue<TMessage, TValue>(TMessage message, PropertyInfo propertyInfo)
        {
            return (TValue) propertyInfo.GetValue(message, new object[0]);
        }
