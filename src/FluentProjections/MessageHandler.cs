...
namespace FluentProjections
{
    ...
    public abstract class MessageHandler<TProjection> where TProjection : class, new()
    {
        ...
        protected async Task HandleAsync<TMessage>(
            TMessage message,
        ...
        )
        {
            ...
            try
            {
                await handler.HandleAsync(message, projections);
                if (projections is IUnitOfWork uow) await uow.CommitAsync();
            }
            finally
            {
        ...
        private class Handlers
        {
            ...
            {
                object handler;
                if (_handlers.TryGetValue(configurer, out handler)) return (IHandleMessages<TMessage>) handler;
                handler = new MessageHandler<TMessage>(configurer);
                if (_handlers.TryAdd(configurer, handler)) return (IHandleMessages<TMessage>) handler;
                if (_handlers.TryGetValue(configurer, out handler)) return (IHandleMessages<TMessage>) handler;
            ...
            private class MessageHandler<TMessage> : IHandleMessages<TMessage>
            {
                ...
                public async Task HandleAsync(TMessage message, IProvideProjections projections)
                {
                    EnsureStrategy();
                    await _strategy.HandleAsync(message, projections);
                }
                ...
                {
                    if (_factoryContainer != null) return;
        ...
        {
            Task HandleAsync(TMessage message, IProvideProjections projections);
        }
    }
}